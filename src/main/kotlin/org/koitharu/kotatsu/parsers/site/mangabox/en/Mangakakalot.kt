...
internal class Mangakakalot(context: MangaLoaderContext) : MangaboxParser(context, MangaParserSource.MANGAKAKALOT) {
	...
	override suspend fun getListPage(page: Int, order: SortOrder, filter: MangaListFilter): List<Manga> {
		val url = if (!filter.query.isNullOrEmpty()) {
			val titleName = filter.query.toQueryParam()
			if (titleName.isNotBlank()) {
				"https://${domain}${searchUrl}${titleName}/?page=$page"
			} else {
				// Fallback if query is empty after sanitization, or just default list
				"https://$domain/genre/all?page=$page&type=${order.toQueryParam()}"
			}
		} else {
			buildString {
