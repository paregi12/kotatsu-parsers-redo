...
internal class MangaBall(context: MangaLoaderContext) : PagedMangaParser(context, MangaParserSource.MANGABALL, 20) {
    ...
    private suspend fun getCsrfToken(forceRefresh: Boolean = false): String {
        ...
        val token = doc.selectFirst("meta[name=csrf-token]")?.attr("content")
            ?: throw ParseException(shortMessage = "CSRF token not found", url = url)
        csrfToken = token
        return token
    }
    ...
    override suspend fun getListPage(page: Int, order: SortOrder, filter: MangaListFilter): List<Manga> {
        ...
        filter.types.firstOrNull()?.let {
            form["filters[originalLanguages]"] = when (it) {
    ...
    override suspend fun getDetails(manga: Manga): Manga {
        ...
        val chaptersData = chapterJson.optJSONArray("ALL_CHAPTERS") ?: return manga.copy(chapters = emptyList())
        ...
        for (i in 0 until chaptersData.length()) {
            ...
            for (j in 0 until translations.length()) {
                ...
                val date = runCatching { dateFormat.parse(dateStr)?.time }.getOrNull() ?: 0L
