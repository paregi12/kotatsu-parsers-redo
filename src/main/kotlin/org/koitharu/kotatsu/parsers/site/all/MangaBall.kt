...
internal class MangaBall(context: MangaLoaderContext) : PagedMangaParser(context, MangaParserSource.MANGABALL, 20) {
    ...
    override val filterCapabilities: MangaListFilterCapabilities
        get() = MangaListFilterCapabilities(
            ...
            isOriginalLocaleSupported = true, // Kotatsu uses this for Translated Locale too in some sources
        )
        ...
        for (i in 0 until chaptersData.length()) {
            ...
            for (j in 0 until translations.length()) {
                ...
                chapters.add(MangaChapter(
                    id = generateUid(chId),
                    title = if (title.contains(number.toString())) title else "Ch. ${number.formatSimple()} $title",
                    number = number,
                    volume = 0,
                    url = chId,
                    scanlator = group,
