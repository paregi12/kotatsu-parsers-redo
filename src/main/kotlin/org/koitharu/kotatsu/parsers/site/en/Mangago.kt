...
internal class MangaGo(context: MangaLoaderContext) :
	...
	override suspend fun getPages(chapter: MangaChapter): List<MangaPage> {
		...
		val jsContent = deobfuscateSoJsonV4(jsContentObfuscated)

		// 3. Extract AES Key and IV using Regex
		val keyHex = CRYPTO_HEX_REGEX.findAll(jsContent)
			.elementAtOrNull(0)?.groupValues?.get(1) ?: "e11adc3949ba59abbe56e057f20f883e"

		val ivHex = CRYPTO_HEX_REGEX.findAll(jsContent)
		...
		val keyLocations = STR_CHAR_AT_REGEX.findAll(jsContent)
		...
		// 6. Extract Puzzle Keys and Grid Size
		val imgKeyKeys = IMG_KEY_KEYS_REGEX.findAll(jsContent)
			.map { it.groupValues[1] to it.groupValues[2] }.toMap()

		val cols = GRID_SIZE_REGEX.find(jsContent)?.groupValues?.get(1) ?: "9"

		return finalUrlString.split(",").map { url ->
			val finalUrl = if (url.contains("cspiclink")) {
				val puzzleKey = imgKeyKeys.entries.find { url.contains(it.key) }?.value
				if (puzzleKey != null) {
	...
	override fun intercept(chain: Interceptor.Chain): Response {
		...
		if (fragment?.contains("desckey=") == true) {
			...
			val cols = fragment.substringAfter("cols=", "9").substringBefore("&").toInt()
			...
			return context.redrawImageResponse(response) { bitmap ->
				...
				val smHeight = height / cols // Typically square grid
	...
	companion object {
		...
		private val IMG_KEY_KEYS_REGEX = Regex("""_img[a-f0-9]{5,}\[['"]([^'"]+)['"]\]\s*=\s*['"]([^'"]+)['"]""")
