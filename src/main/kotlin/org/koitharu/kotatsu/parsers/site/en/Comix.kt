...
internal class Comix(context: MangaLoaderContext) :
	...
	private suspend fun getChapters(hashId: String): List<MangaChapter> = coroutineScope {
		...
		if (totalPages > 1) {
			...
			}.awaitAll().filterNotNull().forEach { arr ->
				for (i in 0 until arr.length()) {
					arr.optJSONObject(i)?.let { allItems.add(it) }
				}
			}
		}

		return@coroutineScope allItems.mapNotNull { item ->
			...
			MangaChapter(
				...
				branch = scanlatorName // Enabling scanlators floating window
			)
		}.sortedBy { it.number }
	}
	...
	private suspend fun fetchAvailableTags(): Set<MangaTag> {
		...
		suspend fun fetch(type: String, page: Int = 1) {
			val url = "$apiBaseUrl/terms".toHttpUrl().newBuilder()
				...
				.addQueryParameter("page", page.toString())
				.build()
			val resp = try { webClient.httpGet(url).parseJson() } catch (_: Exception) { return }
			...
			for (i in 0 until items.length()) {
				...
				tags.add(MangaTag(id, title, source))
			}
			val pagination = result.optJSONObject("pagination")
