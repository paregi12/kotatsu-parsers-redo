...
internal class Manhastro(context: MangaLoaderContext) :
    ...
    private suspend fun fetchChapters(mangaId: Int): List<MangaChapter> {
        ...
        for (i in 0 until data.length()) {
            ...
            val date = try {
                if (dateStr.isNotEmpty()) dateFormat.parse(dateStr)?.time ?: 0L else 0L
            } catch (e: Exception) {
                0L
            }
    ...
    override suspend fun getPages(chapter: MangaChapter): List<MangaPage> {
        ...
        // Skip novels (text: true) as Kotatsu expects images
        ...
        companion object {
            fun fromJson(json: JSONObject): MangaDto {
                ...
                if (genresStr.startsWith("[")) {
                    ...
                    genresList = genresStr.split(",").map { it.trim() }.filter { it.isNotEmpty() }
                } else if (genresStr.isNotBlank()) {
                    genresList = listOf(genresStr.trim())
                }
