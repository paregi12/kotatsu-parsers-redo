...
internal class BatoToV4Parser(context: MangaLoaderContext) : PagedMangaParser(
	...
	override suspend fun getFilterOptions(): MangaListFilterOptions {
		return MangaListFilterOptions(
			availableTags = GENRE_OPTIONS.mapToSet { (title, key) -> MangaTag(key, title, source) },
	...
	override suspend fun getListPage(page: Int, order: SortOrder, filter: MangaListFilter): List<Manga> {
		val variables = JSONObject().apply {
			val select = JSONObject().apply {
				...
				put("incOLangs", JSONArray()) // Original language not directly mapped in filter
				put("incTLangs", JSONArray(filter.locale?.let { listOf(it.language) } ?: emptyList()))
				put("origStatus", filter.states.oneOrNull()?.let {
					when (it) {
...
MangaChapter(
	...
	private suspend fun graphQLQuery(endpoint: String, query: String, variables: JSONObject): JSONObject {
		...
		val response = webClient.httpPost(endpoint, payload.toString().toRequestBody(mediaType), getRequestHeaders())
