...
internal class MangaPark(context: MangaLoaderContext) :
    ...
    // Helper to prevent "Value Error" crashes
    private fun Response.parseJsonSafe(): JSONObject {
        val bodyText = body?.string() ?: ""
        if (bodyText.isEmpty()) throw Exception("Empty response from server")
        try {
            ...
            throw Exception("Failed to parse JSON (Server might be blocking): $bodyText")
        }
    }
    ...
    // String extension helper for case conversion
